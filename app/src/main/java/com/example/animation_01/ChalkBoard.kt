...
class ChalkBoard//Constructor - initialize this View
    ...
    fun wander() {
        ...
        when (style) {
            // Smooth Move ObjectAnimator
            ANIMATOR -> getObjectAnimator(500, "fraction", 0.0f, 1.0f).start() //local method
            ...
            SPIN -> {
                val bounce = getObjectAnimator(8000, "fraction", 0.0f, 1.0f) //local method
                val newcolor = getObjectAnimator(4000, "currColor", 0.0f, 1.0f)
                val rotate = getObjectAnimator(4000, "angle", 0.0f, 180.0f)
                val bounceRotate = AnimatorSet()
                bounceRotate.interpolator = BounceInterpolator()
                bounceRotate.play(rotate).with(newcolor).with(bounce)
                bounceRotate.start()
            }
            TAKEOFF  -> {
                anim = getObjectAnimator(2000, "fraction", 0.0f, 5.0f) //local method
                anim.interpolator = AccelerateInterpolator(10.5f) //try 1.5f or 0.8f
                anim.start()
            }
            else -> {
            }
    ...
    companion object {  //static stuff
        ...
        const val TAKEOFF = 6  //Constant to indicate rotate around View center animation
        const val SPIN = 7  //Constant to indicate rotate around View center animation
        ...
        const val MOVE_ROTATE_RECOLOR = 23  //Constant to indicate move and rotate simultaneously
        // then recolor animation
    }
}